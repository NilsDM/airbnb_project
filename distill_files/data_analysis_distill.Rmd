...
To get the most common room type available we start by selecting `room_type` and `has_availability`, the last one being a logical indicator of general unit availability of room types. Then we filter the rooms that have availability, group them by `room_type` and ranked the room types based.
...
For better understanding of room type availability we plotted a bar graph.
...
As we see "Entire home/apt" is the room type with the most units available overall but that is also affected by the amount of "Entire home/apt" that are listed on Airbnb. When we take a look at the data there is also information about availability over periods of 30, 60, 90 and 365 days, this allow us to understand the average amount of nights available per room type on a selected period.
...
As we see in the plot "Entire home/apt" are the room types with the least average availability, combined with the fact that "Entire home/apt" is the room type with the highest units available we can assume that this is also the room type with the highest demand. 


### Top/Bottom Property Types Based on Average Price

<center>  _Which are top 10 most expensive and top 10 cheapest property type?_ </center>

First we start by selecting `property_type` and `price` from all available data, then we group by property type and summarise by average price. Then we will proceed to create two ranks, ] `top_property_type_average_price` and `bottom_property_type_average_price`.
...
Then we plotted both ranks and using `grid.arrange()` function from the `gridExtra` library to have a parallel view of both plots.
...
For this query we used the same as the previous question but instead of selecting `property_type` and `price` we selected `property_type` and `review_scores_rating` to properly answer this question.
...
Then we created multiple columns according to the max amount of amenities a property can have and populate the columns with all the amenities a property can have. We do this by using `separate`function from the `tidyr`library.
...
By using `pivot_longer` we will prepare our data into Data Base format which will allow us to pipe easily.
...
Now we pipe to answer the question we have and plot like we did on previous queries.
